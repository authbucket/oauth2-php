  ...
  public function validateRequest(Application $app)
  {
    ...
    switch ($filtered_query['grant_type']) {
      case 'authorization_code':
        $grant_type = new AuthorizationCodeGrantType($app);
        $grant_type->buildType($_POST, $filtered_query);
        break;
      case 'client_credentials':
        $grant_type = new ClientCredentialsGrantType($app);
        $grant_type->buildType($_POST, $filtered_query);
        break;
      case 'password':
        $grant_type = new PasswordGrantType($app);
        $grant_type->buildType($_POST, $filtered_query);
        break;
      case 'refresh_token':
        $grant_type = new RefreshTokenGrantType($app);
        $grant_type->buildType($_POST, $filtered_query);
        break;
    }
    return $grant_type;
